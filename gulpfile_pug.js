...
let path = {                                     
    ...
    src:{                                     //путь к исходникам
        ...
        css: source_folder + "/sass/style.sass",     //указываем конкретный scss файл, чтобы gulp обрабатывал только его      
...
function pug() {
    return src(path.src.html)
        ...
        .pipe(
            gulp_pug({
                pretty: true      //чтобы скомпилированный html файл не был в одну строку
            })
        )
        .pipe(dest(path.build.html))
        .pipe(browsersync.stream());
...
let build = gulp.series(clean, gulp.parallel(js, php, plugins, css, pug, images, icons));
